...
3. **The title encodes the positioning.** The title should NAME what frame this angle takes — "The premium-mainstream story", "Why FMCG keeps missing Tier-2", "The Gen-Z packaging gap", "What investors are pricing in that the brand team hasn't yet". The reader should know from the title alone which version of the truth they're stepping into. Avoid generic punch-line titles that don't telegraph the frame. Positioned titles beat punchy-but-empty titles every time.
4. **Lede is one or two sentences max.** It's the opening of the eventual article — the moment that hooks the reader. Specific finding + tension + audience-relevance.
5. **beats: 3 short paragraphs.** Each is a story beat, not a bullet point. Together they sketch the article's arc.
6. **omits is mandatory and honest.** What does this angle deliberately leave out? Which findings are de-emphasised? Which segments are not represented? What's the trade-off of leading with this frame? "Omits" empty or generic is rejected — be specific. Note: the omission is a *positioning choice*, not a confession. "The premium-mainstream story" deliberately doesn't cover budget-conscious segments — that's the point of the angle, not a flaw.
7. **Each angle MUST cite at least one supporting hypothesis_id OR one emergent_pattern.** Pure speculation is rejected. Use the evidence available; if the evidence is thin, mark priority 1-2 and say so in the lede.
8. **Priority 5 = the angle most likely to land with its named audience.** Reserve sparingly. Not every angle is a 5.
...
- target_audience: one sentence. Named segment + their job-to-be-done. Different from the audience of every other angle.
