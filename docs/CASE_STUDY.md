...
- *(2026-04-30)* Phase 2 (hypothesis generation) shipped in roughly the time Phase 1 took, despite being a larger feature. The reason: **everything reused the strict-output chassis from RAG**. Same retrieval, same reranker, same forced tool_use, same defensive post-validation. Only the schema and the system prompt are new. The PM-relevant insight: AI products compound by *primitives*, not by features. Find the load-bearing primitive, name it, reuse it everywhere. Each later phase ships faster *and* feels more coherent to the user because every surface speaks the same structural language.
- *(2026-04-30)* Phase 3 (personas + questionnaires with variants) shipped same-day. The product-defining moment: each question now comes with 3 phrasing variants from different methodological frames (neutral_direct, behavioural, projective, etc.), each labelled with what it elicits and its caveat. The researcher picks based on instinct. The structured-output chassis kept doing its job — variants are enforced by the schema, not by the prompt. The bigger PM insight: *for expert users, don't ship a bot that thinks for them; ship a bot that thinks alongside them.* The "propose, don't dispose" principle is a trust mechanism, not a UX nicety — it's why the expert keeps the bot in their loop even when they disagree.
- *(2026-04-30)* Stopped to do a five-lens audit of everything built so far before continuing. Found 38 specific gaps. The biggest realisation: prompt caching is not actually implemented — D-003 was aspirational. Real cost is roughly 9× what the case study claims. Honest catch. Other Critical findings: zero tests, untested confidentiality, no eval harness. The PM lesson: **build in audit checkpoints, not just feature checkpoints.** Senior teams self-audit per-feature; junior teams find out about the gaps from a hiring manager or client. Audit log lives at `docs/EVALUATION_LOG.md`.
- *(2026-04-30)* Eval harness shipped — six probe types (golden-qa, abstention, hallucination, hypothesis-quality, persona-quality, confidentiality), 20 probes, dedicated test corpus + projects, one CLI. The most AI-PM-credentialing thing in the repo. Even the act of writing the probes was a forcing function — designing a hallucination probe makes you confront whether your system actually catches the failure mode it claims to. Several test cases were sharper than the ones I would have run manually because the JSON forced specificity. Also: every probe is a receipt for a load-bearing claim from D-001 through D-019 — strict abstention, SQL-bounded confidentiality, citation discipline, persona under_represents. Anything we don't have a probe for is something we're trusting on vibes.
- *(2026-04-30)* Tier 1 of the post-audit build queue shipped same-day. Four pieces, one push: **prompt caching** (D-021) — every Anthropic call now uses `cache_control`, real cost matches the docs for the first time; **survey export** (D-022) — markdown / Qualtrics / plaintext, closes the value chain; **cost telemetry** (D-023) — `api_calls` table, live cost badge, the "<$5/mo" claim is now a measurable number not a vibe; **edit affordance** (D-024) — inline editing of hypothesis statements and question variant statements, closing the last 5% by letting the expert finish the work in place. Big PM lesson from this push: **the audit was the unlock.** Stopping mid-feature to audit revealed not just gaps but the *order* in which to close them. The 5.5-day Tier 1+2 plan came directly from the audit; without that pause we'd be 3 phases deeper with all the same gaps still hidden. The pattern: audit-then-build-then-audit, not just build.
- *(2026-05-01)* **Premise went live.** Repo flipped public on GitHub; deployment shipped to Vercel free tier at `premise-one.vercel.app`. Six trust commitments now ride alongside the public surface: prompt caching (D-021), survey export (D-022), cost telemetry (D-023), inline editing (D-024), Zod + safe-error at every API boundary (D-025), atomic generation (D-026), retry logic (D-027), idempotency locks (D-028), in-UI project creation (D-029), public/semi-public deployment strategy (D-030), multi-format ingestion — PDF/DOCX/URL (D-031), Supabase Auth with magic links + per-user ownership + orphan-claim (D-032), and a public library that solves the cold-start problem so first-time users get useful hypotheses on day one (D-033). The PM-relevant insight: **going live is not a feature ship — it's a posture shift**. Pre-live: every commit can be a draft. Post-live: every commit is an action a real user is one click away from experiencing. Decision log + eval harness + cost telemetry stop being case-study artefacts and start being operational defenders. The discipline of building each was preparation for this moment, not paperwork around it.
