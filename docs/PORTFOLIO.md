...
The product is engineered against its single biggest failure mode — fabrication. Every claim is structurally required to cite a source chunk; an independent verifier pass drops unsupported claims rather than rewriting them; the UI refuses to render any uncited output. Zero fabrication is the floor; calibrated estimation with bounded uncertainty (explicit confidence levels per claim) is what sits on top of it.
...
**The strict-abstention architecture (the floor, not the ceiling).** This is the load-bearing piece, and it taught me the most about AI product engineering. Prompts saying "please don't hallucinate" are soft constraints. To make non-fabrication a *guarantee* rather than a *tendency*, Premise enforces it in three structural layers: (a) Anthropic tool-use forces every model output into a JSON schema where each claim must have non-empty `citation_ids`; (b) a second model pass verifies, per claim, that the cited chunks actually support what the claim says — failures are dropped, not rewritten; (c) the UI refuses to render any claim without a citation. Three layers, each cheap, each catching what the previous missed. That's the *floor* — the bot can't fabricate. The *ceiling* is calibrated estimation: each surviving claim carries an explicit confidence level, and below-floor questions return an honest abstention that names what would close the gap. Not "I refuse to help" — "based on the corpus, here's what I can ground, here's what I can't, and here's what would change the answer."
